...
function ProductCard({ product }) {
  ...
  return (
    <div className="card p-3 sm:p-4 hover:shadow-md transition-shadow cursor-pointer" onClick={() => navigate(`/result/${encodeURIComponent(product.name)}`)}>
      ...
      <h3 className="font-poppins font-semibold text-navy mb-1 text-sm sm:text-base line-clamp-2">{product.name}</h3>
      {product.brand && (
        <p className="text-xs text-primary uppercase mb-2 truncate">{product.brand}</p>
      )}
      {product.category && (
        <span className="inline-block text-xs bg-primary-light text-primary px-2 py-1 rounded-full mb-3 truncate max-w-full">
          {product.category}
        </span>
      )}
